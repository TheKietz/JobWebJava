...
<head>
    ...
    <style>
        .status-badge {
            ...
            min-width: 90px; /* hoặc width: 90px; */
            text-align: center;
            font-size: 0.8rem;
        }
    </style>
                                    ...
                                    <td>
                                        <c:choose>
                                            ...
                                            <c:when test="${job.status=='PENDING'}">
                                                <span class="badge bg-info text-dark status-badge">Chờ duyệt</span>
                                            </c:when>
                                            <c:when test="${job.status=='REJECTED'}">
                                                <span class="badge bg-danger status-badge">Bị từ chối</span>
                                            </c:when>
                                            <c:when test="${job.status=='EXPIRED'}">
                                                <span class="badge bg-warning text-dark status-badge">Hết hạn</span>
                                            </c:when>
                                            <c:otherwise>
                                                <span class="badge bg-info text-dark status-badge">${fn:escapeXml(job.status)}</span>
                                            </c:otherwise>
                                        </c:choose>
